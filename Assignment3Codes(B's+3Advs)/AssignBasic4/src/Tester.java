...
public class Tester 
{
	...
	public static void main(String[] args) 
	{
       ...
       /*  while(true)
         ...
         while(true)
         {
        	 ...
        	 subtract=Integer.parseInt(sc.nextLine());
        	 at=Integer.parseInt(sc.nextLine());
        	 if (subtract==-5)
        		 break;
        	 
        	 
        	 decreaseKeyValue(at,heap,subtract);
        	 showheap(heap);
         }
         sc.close();
	}
	...
	public static int[] increaseKeyValue(int i,int[] heap, int add)
	{
		...
		//now resolve the possible problem upwards.
	     ...
	     while (heap[i]>heap[(i-1)/2] && i>0) //while problem exists.
	     {
	...
	public static int[] decreaseKeyValue(int i,int[] heap, int subtract)
	{
		...
		heap[i]=heap[i]-subtract; //modify the keyvalue of node i.
		
		//there may be a problem downwads, but lhs and rhs subtrees are still heaps, so we can call heapify.
