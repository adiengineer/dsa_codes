...
public class Bst 
{
	...
	public int depth(Node curr)
   {
	...
	//method should set two links which will give 3 equal sized subtrees
	public void equaltripartition()
	{
         //first calculate sizes of all subtress.
		
		int n= this.calcnodesubsize(this.root);  //total number of nodes in tree. find subtree of +-n/3
		int target=n/3;  
	        
		this.findsubofsize(this.root,target); //this will store link location in linkpoint varialble defined in code.
		...
		//if found, now temporarily set the sibtree at linknode to be null, to find other parts 
		//and recompute sizesofsub of ancestors of linkpoint, as only they will change.
		
	if (linkpoint.key<linkpoint.parent.key)
	    {isLeftchild=true;
	    linkpoint.parent.left=null; //temporarily neglect this subtree
	    }
	else
	{
		isLeftchild=false;
	    linkpoint.parent.right=null; //temporarily neglect this subtree
	}
	
	Node x=linkpoint.parent;
	
	Node linkpoint1=linkpoint;
	//recompute subsizes up the path as they will change
		while (x!=null)
	{
	     x.sizeofsub=this.calcnodesubsize(x);
	     x=x.parent;
	}
		
		//now find another 2nd fragement of size approx n/3
	      this.findsubofsize(this.root,target);
	      
	Node  linkpoint2=linkpoint; //this will give location of the second link if possible.
	
	if(linkpoint1!=null)
		System.out.println(linkpoint1.key);
	
	if(linkpoint2!=null)
		System.out.println(linkpoint2.key);
	else
		System.out.println("Not possible");
	
	//rejoin fragment which was removed.
	if (isLeftchild==true)
		linkpoint.parent.left=linkpoint;
	else
		linkpoint.parent.right=linkpoint;
	}
	
	
	//calculates subtree size at each node
	public int calcnodesubsize(Node curr)
	{
		if (curr==null)
			return 0;
		...
		if (curr.left==null && curr.right==null)
	     ...
	     curr.sizeofsub=1+ calcnodesubsize(curr.right)+ calcnodesubsize(curr.left);
	     return curr.sizeofsub;
	}
	
	//represents  one vertex of the link required in AdvQ3
	Node linkpoint; 
	boolean isLeftchild;
	//this method will record the vertex of the link which has a n/3 sized subtree.
	
	public void findsubofsize(Node curr,int target) //ini call on root.
	{
		  if (curr==null)
			  return;  //nothing to be found
		  
		  if (curr.sizeofsub==target||curr.sizeofsub==target-1|| curr.sizeofsub==target+1)
		  {
	           linkpoint=curr;
	           return;
		  }
		  
		 findsubofsize(curr.left,target);
		 findsubofsize(curr.right,target);
	}
}
